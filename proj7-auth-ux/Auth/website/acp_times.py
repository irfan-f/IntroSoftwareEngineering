...
def open_time(control_dist_km, brevet_dist_km, brevet_start_time):
    ...
    for from_dist, to_dist, speed in max_speed:
        ...
        if distance_left > seg_length:
            ...
            open_time = start_time.replace(hour=int(elapsed_hours))
...
def close_time(control_dist_km, brevet_dist_km, brevet_start_time):
    ...
    if control_dist_km >= brevet_dist_km:      
        ...
        if duration == 13.5:
            finish_time = start_time.replace(minute=30)
        finish_time = start_time.replace(hour=int(duration))
    ...
    for from_dist, to_dist, speed in min_speed:
        ...
        if distance_left > seg_length:
            ...
            elapsed_hours += distance_left / speed
            cut_time = start_time.replace(hour=int(elapsed_hours))
